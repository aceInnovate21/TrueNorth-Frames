...
// PATCH — toggle whether Google review snippets show on the public profile.
// Body: { showGoogleReviews: boolean }
export async function PATCH(request: NextRequest) {
  ...
  const body = await request.json().catch(() => ({}))
  const show = !!body?.showGoogleReviews
  ...
  await db.from('photographer_profiles')
    .update({ show_google_reviews: show })
    .eq('id', profile.id)

  return NextResponse.json({ success: true, show_google_reviews: show })
}
