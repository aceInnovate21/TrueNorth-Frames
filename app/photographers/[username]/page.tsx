...
import { MessageGate } from '@/components/message-gate'
...
function PackageCard({ pkg, username }: { pkg: any; username: string }) {
  return (
    <div className={`relative rounded-2xl p-5 border transition-all duration-200 hover:-translate-y-0.5 ${
      pkg.isPopular
        ? 'bg-ink border-ink text-white'
        : 'bg-white border-ink-100 hover:border-ink-300'
    }`}
      style={{ boxShadow: pkg.isPopular ? '0 8px 24px rgba(0,0,0,0.18)' : '0 2px 8px rgba(0,0,0,0.04)' }}>
      {pkg.isPopular && (
        <div className="absolute -top-3 left-4">
          <span className="text-[10px] font-bold bg-white text-ink px-3 py-1 rounded-full shadow-sm">Most Popular</span>
        </div>
      )}
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          <p className={`font-semibold text-sm ${pkg.isPopular ? 'text-white' : 'text-ink'}`}>{pkg.name}</p>
          {pkg.description && <p className={`text-xs mt-1 leading-relaxed ${pkg.isPopular ? 'text-white/70' : 'text-ink-400'}`}>{pkg.description}</p>}
        </div>
        <div className="text-right flex-shrink-0">
          <p className={`font-bold text-xl leading-none ${pkg.isPopular ? 'text-white' : 'text-ink'}`}>${pkg.price}</p>
          <p className={`text-[10px] mt-0.5 ${pkg.isPopular ? 'text-white/50' : 'text-ink-300'}`}>/{pkg.billingType === 'hourly' ? 'hr' : 'session'}</p>
        </div>
      </div>
      {pkg.deliverables?.length > 0 && (
        <ul className="space-y-1.5 mb-4">
          {pkg.deliverables.map((d: string, i: number) => (
            <li key={i} className={`flex items-start gap-2 text-xs ${pkg.isPopular ? 'text-white/80' : 'text-ink-500'}`}>
              <CheckCircle2 className={`w-3.5 h-3.5 mt-0.5 flex-shrink-0 ${pkg.isPopular ? 'text-white/60' : 'text-ink'}`} />
              {d}
            </li>
          ))}
        </ul>
      )}
      <MessageGate photographerName="" username={username} variant="package" />
    </div>
  )
}
...
export default function ProfilePage({ params }: { params: { username: string } }) {
  ...
  const trustSources = Object.entries(p.links as Record<string, { url: string; rating: number | null; count: number | null }>)
      ...
      <div className="bg-white border-b border-ink-100" style={{ boxShadow: '0 1px 0 rgba(0,0,0,0.04)' }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-end gap-4 -mt-12 pb-4 relative z-10">
            ...
            {/* Available pill */}
            <div className="mb-2 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-100"
              style={{ boxShadow: '0 1px 4px rgba(16,185,129,0.12)' }}>
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-[11px] font-semibold text-emerald-600 tracking-wide">Available</span>
            </div>
          </div>
          ...
          <div className="pb-5">
            <div className="flex flex-wrap items-start justify-between gap-4 mb-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    {p.instagram_url && (
                      <a href={p.instagram_url} target="_blank" rel="noopener noreferrer"
                        className="w-7 h-7 rounded-lg bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center hover:scale-105 hover:shadow-md transition-all"
                        title="Instagram">
                    ...
                    {p.links?.google?.url && (
                      <a href={p.links.google.url} target="_blank" rel="noopener noreferrer"
                        className="w-7 h-7 rounded-lg bg-white border border-ink-100 flex items-center justify-center hover:scale-105 hover:shadow-md hover:border-blue-200 transition-all"
                        title="Google Reviews">
                        <span className="text-[11px] font-black text-blue-500">G</span>
                      </a>
                    )}
                    {p.website_url && (
                      <a href={p.website_url} target="_blank" rel="noopener noreferrer"
                        className="w-7 h-7 rounded-lg bg-ink-50 border border-ink-100 flex items-center justify-center hover:scale-105 hover:shadow-md hover:border-ink-300 transition-all"
                        title="Website">
              ...
              <div className="flex items-center gap-2 flex-shrink-0 mt-1">
                <SaveButton slug={params.username} />
                <MessageGate photographerName={p.display_name} username={params.username} variant="header" />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 -mb-px">
            {(['overview', 'portfolio'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                ...
                {t === 'portfolio' ? `Portfolio${hasPortfolio ? ` (${standalonePhotos.length + standaloneVideos.length + portfolioAlbums.length})` : ''}` : 'Overview'}
              </button>
            ))}
          </div>
        </div>
            ...
            <div className="max-w-3xl mx-auto">
              {hasPortfolio ? (
                <PortfolioGrid
                  ...
                  allPhotos={portfolioPhotos}
                  allVideos={portfolioVideos}
                />
              ) : (
                <div className="flex flex-col items-center justify-center py-24 text-center">
                  <div className="w-16 h-16 rounded-2xl bg-white border border-ink-100 flex items-center justify-center mb-4"
                    style={{ boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
                    <Camera className="w-8 h-8 text-ink-200" />
                  </div>
                  <p className="text-ink-400 font-medium">No portfolio photos yet</p>
                  <p className="text-ink-300 text-sm mt-1">Check back soon — this photographer is still building their portfolio.</p>
                </div>
              )}
            </div>
          )}
          ...
          {tab === 'overview' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-5">
                ...
                {hasPortfolio && (
                  <section className="bg-white rounded-2xl p-5 border border-ink-50"
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-ink-300 text-xs">
                          {standalonePhotos.length + standaloneVideos.length > 0
                            ? `${standalonePhotos.length + standaloneVideos.length} posts`
                            : ''}
                          {portfolioAlbums.length > 0
                            ? `${standalonePhotos.length + standaloneVideos.length > 0 ? ' · ' : ''}${portfolioAlbums.length} album${portfolioAlbums.length !== 1 ? 's' : ''}`
                            : ''}
                        </span>
                      </div>
                      <button onClick={() => setTab('portfolio')}
                        className="text-xs font-medium text-ink-400 hover:text-ink transition-colors flex items-center gap-1">
                    ...
                    {portfolioAlbums.length > 0 && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-3">
                        {portfolioAlbums.slice(0, 3).map(album => {
                          const srcs = album.collage_srcs?.length ? album.collage_srcs : album.cover_src ? [album.cover_src] : []
                          return (
                            <button key={album.id} type="button" onClick={() => setTab('portfolio')}
                              className="relative aspect-square rounded-xl overflow-hidden bg-ink-100 group cursor-pointer block w-full">
                              {srcs.length >= 4 ? (
                                <div className="grid grid-cols-2 gap-0.5 w-full h-full">
                                  {srcs.slice(0, 4).map((s, i) => (
                                ...
                                <div className="w-full h-full flex items-center justify-center bg-ink-100">
                                  <Camera className="w-6 h-6 text-ink-200" />
                                </div>
                              )}
                              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent pointer-events-none" />
                              <div className="absolute bottom-0 left-0 right-0 p-2 pointer-events-none">
                                <p className="text-white text-[10px] font-semibold truncate">{album.title}</p>
                              </div>
                            </button>
                          )
                        })}
                    ...
                    {/* Standalone photos strip */}
                    {standalonePhotos.length > 0 && (
                      <div className="grid grid-cols-4 sm:grid-cols-6 gap-1.5">
                        {standalonePhotos.slice(0, 6).map(ph => (
                          <button key={ph.id} type="button" onClick={() => setTab('portfolio')}
                            className="aspect-square rounded-lg overflow-hidden bg-ink-100 group">
                  ...
                  {hasPackages ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {p.packages.map((pkg: any) => (
                        <PackageCard key={pkg.id} pkg={pkg} username={params.username} />
                      ))}
                    </div>
                  ) : (
                ...
                {hasAvailability && (
                  <section className="bg-white rounded-2xl p-6 border border-ink-50"
                    ...
                    <div className="mt-5 pt-4 border-t border-ink-50">
                      <MessageGate photographerName={p.display_name} username={params.username} variant="booking" />
                    </div>
                  </section>
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-2xl p-5 text-white overflow-hidden relative"
                  ...
                  <MessageGate photographerName={p.display_name} username={params.username} variant="sidebar" />
                </div>
                ...
                {(p.instagram_url || p.website_url || p.links?.google?.url || p.links?.yelp?.url) && (
                  <div className="bg-white rounded-2xl p-5 border border-ink-50"
                    style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                    <p className="text-xs font-semibold text-ink mb-3 uppercase tracking-wide">Social &amp; Reviews</p>
                    <div className="space-y-2">
                      {p.instagram_url && (
                        <a href={p.instagram_url} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-3 p-2.5 rounded-xl bg-gradient-to-r from-pink-50 to-purple-50 border border-pink-100 hover:border-pink-300 hover:shadow-sm transition-all group">
                          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {p.links.google.rating != null ? (
                              <div className="flex items-center gap-1">
                                ...
                                {p.links.google.count && <span className="text-ink-300 text-[10px]">({p.links.google.count})</span>}
                              </div>
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {p.links.yelp.rating != null ? (
                              <div className="flex items-center gap-1">
                                ...
                                {p.links.yelp.count && <span className="text-ink-300 text-[10px]">({p.links.yelp.count})</span>}
                              </div>
                            ) : <p className="text-xs font-semibold text-ink">View reviews</p>}
                          </div>
                          <ExternalLink className="w-3 h-3 text-ink-200 group-hover:text-ink-400 transition-colors flex-shrink-0" />
                      ...
                      {p.website_url && (
                        <a href={p.website_url} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-3 p-2.5 rounded-xl bg-ink-50 border border-ink-100 hover:border-ink-300 hover:shadow-sm transition-all group">
                          <div className="w-8 h-8 rounded-lg bg-ink flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform">
                ...
                {hasPackages && (
                  <div className="bg-white rounded-2xl p-5 border border-ink-50"
                    style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                    <p className="text-xs font-semibold text-ink mb-3 uppercase tracking-wide">Packages</p>
                    <div className="space-y-2">
                      {p.packages.map((pkg: any) => (
                        <div key={pkg.id} className="flex items-center justify-between py-1">
                          ...
                          <span className="text-xs font-bold text-ink flex-shrink-0 ml-2">
                            ${pkg.price}<span className="text-ink-300 font-normal text-[10px]">/{pkg.billingType === 'hourly' ? 'hr' : 'session'}</span>
                          </span>
                        </div>
                      ))}
                    </div>
