...
import {
  ...
  Instagram, MessageSquare, Star, Send, CheckCircle2, AlertTriangle, ShieldX,
} from 'lucide-react'
...
function ReviewCard({
  ...
  return (
    <div className={`rounded-2xl overflow-hidden border transition-all ${
      review.flag === 'flag_resolved' ? 'border-red-100 bg-red-50/30 opacity-70' :
      review.flag === 'flagged'       ? 'bg-white border-amber-200' :
      isLowRating                     ? 'bg-white border-red-100'   : 'bg-white border-ink-100'
    }`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {review.flag === 'flag_resolved' && (
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full border bg-red-50 text-red-600 border-red-200 flex items-center gap-1">
                <ShieldX className="w-2.5 h-2.5" /> Removed by admin
              </span>
            )}
          </div>
          ...
          {review.flag === 'flag_resolved' && (
            <div className="bg-red-50 border border-red-200 rounded-xl px-3 py-2.5">
              <p className="text-xs font-semibold text-red-700 flex items-center gap-1.5">
                <ShieldX className="w-3.5 h-3.5" /> Removed by admin
              </p>
              <p className="text-[10px] text-red-500 mt-0.5">
                This review was found to violate our policies and has been removed from your public profile.
              </p>
            </div>
          )}
        </div>
